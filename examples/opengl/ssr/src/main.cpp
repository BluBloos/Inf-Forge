...
// clang-format off
  ...
  // add the depth buffer to this framebuffer.
  {
    ...
    glTexImage2D(
        GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT, winInfo.width, winInfo.height, 0, GL_DEPTH_COMPONENT, GL_FLOAT, NULL);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
...
void MonkeyDemoRender(ae::game_memory_t *gameMemory)
{
  ...
  // do a first pass of rendering the geometry just to get the normals in an image for us.
  glBindFramebuffer(GL_FRAMEBUFFER, gameState->gBuffer);
  fnRenderGeometry();

  // reset and normal render the geometry.
